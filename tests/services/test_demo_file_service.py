...
def test_demo_file_exists_returns_true_when_demo_file_exists(tmp_path, monkeypatch):
    demo_file = tmp_path / "data" / "test_data.csv"
...
def test_download_demo_file_copies_demo_file(tmp_path, monkeypatch):
    source_file = tmp_path / "source" / "test_data.csv"
    source_file.parent.mkdir()
    source_file.write_bytes(b"ticket_id,customer\n123,Acme")

    destination_directory = tmp_path / "destination"
    destination_directory.mkdir()

    destination_file = destination_directory / "demo.csv"
    ...
    DemoFileService.download_demo_file(destination_file)
...
def test_download_demo_file_raises_error_when_source_does_not_exist(
    ...
    source_file = tmp_path / "source" / "test_data.csv"
    destination_file = tmp_path / "destination" / "demo.csv"
