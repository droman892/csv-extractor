...
class FakeResultsView(QWidget):
    ...
    def __init__(self, display_result, full_result):
        super().__init__()
...
def test_main_window_initializes_with_upload_view(qtbot):
    ...
    assert isinstance(window.upload_view, UploadView)
    assert window.centralWidget() is window.upload_view
...
def test_show_results_view_stops_processing_overlay(qtbot):
    ...
    assert window.centralWidget() is window.results_view


def test_show_processing_error_stops_overlay_and_forwards_error(qtbot):
    window = MainWindow()
    qtbot.addWidget(window)
    ...
    window.show_processing_error("Processing failed")
...
def test_main_window_resizes_processing_overlay(qtbot):
    ...
    assert window.processing_overlay.geometry() == window.rect()